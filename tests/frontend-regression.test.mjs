...
test("data source card shows public preview recovery access summary", () => {
  ...
  assert.match(html, /连续检查：180 秒 \/ 5 个端点/);
...
test("connected backend news loads through API on startup", async () => {
  ...
  assert.match(app.byId.get("stockCoverageNote").textContent, /新闻更新：2026-06-13 \d{2}:\d{2}/);
  assert.match(app.byId.get("stockCoverageNote").textContent, /公告更新：2026-06-12 \d{2}:\d{2}/);
  assert.match(app.byId.get("stockCoverageNote").textContent, /公开言论更新：2026-06-13 \d{2}:\d{2}/);
  assert.doesNotMatch(app.byId.get("stockCoverageNote").textContent, /新闻更新：06\/13|公告更新：06\/12|公开言论更新：06\/13/);
});
...
test("real-data rule reference analysis is not labeled as full AI generation", async () => {
  ...
  assert.doesNotMatch(app.byId.get("analysisState").innerHTML, /原因：主模型额度不足，备用模型限流或校验失败/);
  assert.doesNotMatch(app.byId.get("analysisState").innerHTML, /查看简要原因/);
  assert.match(app.byId.get("analysisState").innerHTML, /恢复提示[\s\S]*当前没有未冷却备用模型可立即继续检查/);
  assert.match(app.byId.get("analysisState").innerHTML, /当前没有未冷却备用模型可立即继续检查/);
  assert.match(app.byId.get("analysisState").innerHTML, /AI 失败原因分类/);
  assert.match(app.byId.get("analysisState").innerHTML, /主模型额度不足/);
...
test("AI relay slot summary keeps safety failure distinct from provider rate limits", async () => {
  ...
  assert.match(html, /主模型额度不足/);
  assert.match(html, /备用模型限流/);
  assert.match(html, /输出格式错误/);
  assert.match(html, /安全校验失败/);
...
test("AI relay compact summary does not misread safety repair not-required as safety failure", async () => {
  ...
  const html = app.byId.get("analysisState").innerHTML;
  assert.match(html, /完整 AI 暂不可用，当前显示规则参考。/);
  assert.doesNotMatch(html, /原因：主模型额度不足，备用模型限流或校验失败/);
  assert.match(html, /恢复提示[\s\S]*当前没有未冷却备用模型可立即继续检查/);
...
test("automatic real-data ingestion updates latest quote without sample trend wording", async () => {
  ...
  assert.match(app.byId.get("stockCoverageNote").textContent, /行情更新：2026-06-08 \d{2}:\d{2}/);
  assert.doesNotMatch(app.byId.get("stockCoverageNote").textContent, /行情更新：06\/08/);
  assert.equal(app.byId.get("tradePlan").hidden, true);
...
test("automatic free-api relay quote is rendered as live provider data", async () => {
  ...
  assert.match(app.byId.get("stockCoverageNote").textContent, /行情\s*已连接/);
  assert.match(app.byId.get("stockCoverageNote").textContent, /行情更新：2026-06-12 \d{2}:\d{2}/);
  assert.doesNotMatch(app.byId.get("stockCoverageNote").textContent, /行情更新：06\/12/);
  assert.equal(app.byId.get("tradePlan").hidden, true);
  assert.match(app.byId.get("analysisBasis").innerHTML, /行情：backend-real-provider-quote/);
});
...
test("connected backend analysis failure keeps strict real-data empty state", async () => {
  ...
  assert.match(app.byId.get("stockCoverageNote").textContent, /行情更新：2026-06-12 \d{2}:\d{2}/);
  assert.doesNotMatch(app.byId.get("stockCoverageNote").textContent, /行情更新：06\/12/);
  assert.match(app.byId.get("stockCoverageNote").textContent, /宏观更新：2024-12-31 · 年度宏观数据/);
  assert.doesNotMatch(app.byId.get("stockCoverageNote").textContent, /宏观更新：12\/31/);
