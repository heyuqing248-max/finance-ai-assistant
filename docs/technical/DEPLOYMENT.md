...
如果你按 OpenAI、Gemini、OpenRouter、Groq 的顺序重新创建并复制模型 key，Render Environment Variables 左列应按下面 4 行填写；右侧 Value 粘贴对应 key 后点击保存。不要把右侧 Value 写入仓库、文档、截图或聊天记录：

| 顺序 | Render 左列变量名 | 对应模型来源 |
| --- | --- | --- |
| 1 | `FINANCE_AI_MODEL_API_KEY` | OpenAI 主模型 |
| 2 | `FINANCE_AI_MODEL_FALLBACK_API_KEY` | Gemini 备用模型 |
| 3 | `FINANCE_AI_MODEL_FALLBACK2_API_KEY` | OpenRouter 备用模型 |
| 4 | `FINANCE_AI_MODEL_FALLBACK3_API_KEY` | Groq 备用模型 |
...
If you recreated and copied model keys in the order OpenAI, Gemini, OpenRouter, and Groq, the Render Environment Variables left column should use the following four names. Paste the matching key into the right-side Value field, then save. Never store the right-side Value in the repo, docs, screenshots, or chat logs:

| Order | Render left-column variable name | Model source |
| --- | --- | --- |
| 1 | `FINANCE_AI_MODEL_API_KEY` | OpenAI primary model |
| 2 | `FINANCE_AI_MODEL_FALLBACK_API_KEY` | Gemini fallback model |
| 3 | `FINANCE_AI_MODEL_FALLBACK2_API_KEY` | OpenRouter fallback model |
| 4 | `FINANCE_AI_MODEL_FALLBACK3_API_KEY` | Groq fallback model |
